...
class bootloader:
    ...
    def install(self):
        if self.partitiontype == partitiontable.EPartitionTabel.GPT:
            shell.Command(self.installcommand.format(
                self.partitiontable.disk)).GetReturnCode()
        else:
            shell.Command(self.installDOScommand.format(
                self.partitiontable.disk)).GetReturnCode()
